...
def load_seq2seq_model():
    seq2seq_model_path = 'C:/Users/JanRoShin/Desktop/UAQTEbot_django/application/Models/my_model11_20.h5'
...
def get_response_from_seq2seq_model(question, model, max_sequence_length, vocab):
    ...
    # Initialize the output answer
    answer = ''
    # Loop to generate the answer word by word
    for i in range(max_sequence_length - 1):
        # Predict the next word
        predictions = model.predict([padded_question, decoder_input]).argmax(axis=-1)
        # Get the predicted word index
        predicted_word_index = predictions[0, i]
        # Map the predicted word index to the actual word
        predicted_word = inv_vocab.get(predicted_word_index, '<OUT>')
        # Append the predicted word to the answer
...
def start_chat(user_input):
    ...
    seq2seq_model = load_seq2seq_model()
    seq2seq_response = get_response_from_seq2seq_model(user_input, seq2seq_model, 519, vocab)
