...
def find_closest_match(user_input, database):
    ...
    for question in database:
        score = 1 - ld_algo(user_input, question)
        print(
...
def load_seq2seq_model():
    seq2seq_model_path = 'C:/Users/JanRoShin/Desktop/UAQTEbot_django/application/Models/model1000_w_att.h5'
...
def beam_search(model, input_seq, vocab, inv_vocab, max_sequence_length, beam_width=3):
    # Preprocess and tokenize input
    preprocessed_question = preprocess_text_input(input_seq)
    ...
    decoder_input = np.zeros((beam_width, max_sequence_length))
    decoder_input[:, 0] = vocab['<SOS>']
    ...
    for _ in range(max_sequence_length - 1):
        ...
        for seq, score in sequences:
            padded_question_rep = np.repeat(padded_question, len(sequences), axis=0)
            decoder_input_rep = np.array([np.pad(seq, (0, max_sequence_length - len(seq)), 'constant') for seq, _ in sequences])

            predictions = model.predict([padded_question_rep, decoder_input_rep])

            next_word_probs = predictions[:, len(seq)]
            next_word_idx = np.argsort(next_word_probs, axis=-1)[:, -beam_width:]

            # Debugging: Print shapes and other relevant information
            print(f"padded_question_rep shape: {padded_question_rep.shape}")
            print(f"decoder_input_rep shape: {decoder_input_rep.shape}")
            print(f"predictions shape: {predictions.shape}")
            print(f"next_word_probs shape: {next_word_probs.shape}")
            print(f"next_word_idx shape: {next_word_idx.shape}")

            for beam_idx in range(len(next_word_idx)):
                for idx in next_word_idx[beam_idx]:
                    candidate = [seq + [idx], score - np.log(next_word_probs[beam_idx][idx])]
        ...
        for seq, score in sequences:
            if seq[-1] == vocab['<EOS>']:
                end_sequences.append((seq, score))
                sequences.remove((seq, score))

        if not sequences:
            break

    if not end_sequences:
        end_sequences = sequences

    best_seq = sorted(end_sequences, key=lambda tup: tup[1])[0][0]
    response = ' '.join([inv_vocab.get(idx, '<OUT>') for idx in best_seq if idx != vocab['<SOS>'] and idx != vocab['<EOS>']])
    
    return response


def get_response_from_seq2seq_model(question, model, max_sequence_length, vocab, inv_vocab):
    return beam_search(model, question, vocab, inv_vocab, max_sequence_length)
...
def start_chat(user_input):
    ...
    seq2seq_model = load_seq2seq_model()
    seq2seq_response = get_response_from_seq2seq_model(user_input, seq2seq_model, 519, vocab, inv_vocab)
